 ...
 * <p>
 * 功能介绍：标签选择控件，一行个数不定，自动换行，可多选，每行居中排列
 */

public class CustomLabelMultiSelectCenterView extends ViewGroup {
    ...
    public CustomLabelMultiSelectCenterView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }
    ...
    public void setSelect(int index, boolean isSelect) {
        if (getChildCount() > 0) {
            TextView itemView = (TextView) getChildAt(index);
            itemView.setTextColor(isSelect ? 0xffff7640 : 0xffD4C0B7);
            itemView.setBackgroundResource(isSelect ? R.drawable.shape_tv_lable : R.drawable.shape_tv_lable_unselect);
        }
    }
}
